...
└── /main
    ...
    │   │   ├── /util        -> أدوات مساعدة (مدير قاعدة البيانات، المحولات)
...
*   **Service (طبقة الخدمات):**
    ...
    *   **الوصف:** هذه الطبقة هي **عقل التطبيق الحقيقي**. تحتوي على كل منطق العمل والمنطق المحاسبي المعقد. على سبيل المثال، `SaleDataService.java` يحتوي على دالة `addSale` التي لا تقوم فقط بحفظ سجل البيع، بل تقوم أيضًا بحساب تكلفة البضاعة المباعة، وإنشاء جميع القيود المحاسبية المترتبة، وتحديث المخزون، وكل ذلك داخل معاملة (Transaction) واحدة لضمان سلامة البيانات.
...
*   **Util (الأدوات المساعدة):**
    ...
    *   **الوصف:** تحتوي هذه الحزمة على فئات مساعدة مهمة، أهمها `ImprovedDataManager.java` الذي يدير اتصال قاعدة البيانات باستخدام `HikariCP` ويوفر وظيفة `executeTransaction` القوية التي تستخدمها طبقة الخدمات لضمان تكامل البيانات.
